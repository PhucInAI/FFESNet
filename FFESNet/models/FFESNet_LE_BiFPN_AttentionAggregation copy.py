"""
SegFormer Encoder with BiFPN (combined with AttentionAggregation)
"""
...
import mit
from pyramids.bifpn import BiFPN
from pyramids.a2fpn import AttentionAggregationModule
from pyramids.le import LocalEmphasisLayer
...
class FFESNet(nn.Module):
    ...
    def __init__(self, model_type = 'B0', dropout = 0.1, embedding_dim = 160):
        ...
        # Local Emphasis
        # ----------------------------------------------------------------
        self.le1 = LocalEmphasisLayer(self.backbone.embed_dims[0], self.backbone.embed_dims[0])
        self.le2 = LocalEmphasisLayer(self.backbone.embed_dims[1], self.backbone.embed_dims[1])
        self.le3 = LocalEmphasisLayer(self.backbone.embed_dims[2], self.backbone.embed_dims[2])
        self.le4 = LocalEmphasisLayer(self.backbone.embed_dims[3], self.backbone.embed_dims[3])
        ...
        # Prediction
        # ----------------------------------------------------------------
        self.attention = AttentionAggregationModule(256, 256)
    ...
    def _init_weights(self):
        ...
        """
        if self.model_type == 'B0':
            pretrained_dict = torch.load('/home/ptn/Storage/LJMU_Thesis/FFESNet/models/pretrained/mit_b0.pth')
        if self.model_type == 'B1':
            pretrained_dict = torch.load('/home/ptn/Storage/LJMU_Thesis/FFESNet/models/pretrained/mit_b1.pth')
        if self.model_type == 'B2':
            pretrained_dict = torch.load('/home/ptn/Storage/LJMU_Thesis/FFESNet/models/pretrained/mit_b2.pth')
        if self.model_type == 'B3':
            pretrained_dict = torch.load('/home/ptn/Storage/LJMU_Thesis/FFESNet/models/pretrained/mit_b3.pth')
        if self.model_type == 'B4':
            pretrained_dict = torch.load('/home/ptn/Storage/LJMU_Thesis/FFESNet/models/pretrained/mit_b4.pth')
        if self.model_type == 'B5':
            pretrained_dict = torch.load('/home/ptn/Storage/LJMU_Thesis/FFESNet/models/pretrained/mit_b5.pth')
    ...
    def forward(self, x):
        ...
        out = self.attention(p4, p3, p2, p1)
